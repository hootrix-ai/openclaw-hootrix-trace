...
function buildAuditFinding(rule, matches, scanField) {
    const firstMatch = matches[0];
    const positionKey = scanField === "assistantTexts" ? "assistantTexts" : scanField === "tool_params" ? "toolParams" : "prompt";
    const spanField = scanField === "assistantTexts" ? "output_json" : scanField === "tool_params" ? "metadata_json" : "input_json";
    return {
        policy_id: rule.id,
        ...
        policy_action: String(rule.policyAction ?? "").trim().toLowerCase() ||
        ...
        position: {
            [positionKey]: {
                ...
                match_count: matches.length,
                offset: matches.map((m) => [m.start, m.end]),
...
export function mergeAuditFindings(base, incoming) {
    const byId = new Map();
...
export function scanAuditOnlyFindings(text, rules, regexById, scanField) {
...
function redactWithSingleRule(match, rule) {
...
function applyReplace(match, rule) {
    const action = String(rule.policyAction ?? "").trim().toLowerCase() ||
        (rule.redactType === "block" ? "data_mask" : "data_mask");
    switch (action) {
        case "audit_only":
            return match;
 ...
 */
export function processTextSegment(text, rules, regexById, auditScanField) {
    let shadowHits = 0;
    let replacements = 0;
    let block = false;
    let out = text;
...
export function deepSanitizeStrings(input, rules, precompiledRegexById, options) {
