...
export function registerSubagentHooks(deps) {
    ...
    deps.api.on("subagent_spawning", (event, subagentCtx) => {
        ...
        try {
            ...
            deps.warn(`opik: subagent span creation failed (childSessionKey=${childSessionKey}): ${deps.formatError(err)}`);
        }
    ...
    deps.api.on("subagent_spawned", (event, subagentCtx) => {
        ...
        if (!span) {
            ...
            try {
                ...
                deps.warn(`opik: subagent span creation failed on spawn (childSessionKey=${childSessionKey}): ${deps.formatError(err)}`);
                return;
            }
        }
    ...
    deps.api.on("subagent_delivery_target", (event, subagentCtx) => {
        ...
        if (childActive) {
            ...
            try {
                ...
                deps.warn(`opik: subagent delivery target span failed (childSessionKey=${childSessionKey}): ${deps.formatError(err)}`);
            }
            return;
        }
        const existingHost = deps.getSubagentSpanHost(childSessionKey);
        ...
        if (!span) {
            try {
                ...
                deps.warn(`opik: subagent span creation failed on delivery target (childSessionKey=${childSessionKey}): ${deps.formatError(err)}`);
                return;
            }
        }
        const requesterOrigin = eventObj.requesterOrigin && typeof eventObj.requesterOrigin === "object" && !Array.isArray(eventObj.requesterOrigin)
    ...
    deps.api.on("subagent_ended", (event, subagentCtx) => {
        ...
        if (!span) {
            try {
                ...
                deps.warn(`opik: subagent span creation failed on end (targetSessionKey=${targetSessionKey ?? "unknown"}): ${deps.formatError(err)}`);
                return;
            }
        }
        const spanUpdate = {
