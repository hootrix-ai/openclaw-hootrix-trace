...
export function registerToolHooks(deps) {
    ...
    deps.api.on("before_tool_call", (event, toolCtx) => {
        ...
        try {
            ...
            deps.warn(`opik: tool span creation failed (sessionKey=${sessionKey}, tool=${event.toolName}): ${deps.formatError(err)}`);
            return;
        }
    ...
    deps.api.on("after_tool_call", (event, toolCtx) => {
        ...
        if (event.params && typeof event.params === "object" && !Array.isArray(event.params)) {
            spanUpdate.input = sanitizeValueForOpik(event.params);
        }
        const spanMetadata = {
        ...
        if (event.error) {
            ...
            spanUpdate.output = sanitizeValueForOpik(output);
        }
