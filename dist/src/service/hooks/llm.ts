...
export function registerLlmHooks(deps: LlmHooksDeps): void {
  ...
  deps.api.on("llm_input", (event, agentCtx) => {
    ...
    const llmInputForExport =
      Object.keys(sanitizedLlmInput).length > 0 ? sanitizedLlmInput : undefined;
    ...
    let parentTurnIdForAsync: string | undefined;
    ...
    if (existing && capabilities.allowFinalizeReuse) {
      ...
      const lineage =
        classification.traceType === "subagent" ? deps.getSubagentLineage(sessionKey) : undefined;
      const parentTurnId =
    ...
    if (existing) {
      if (llmInputForExport) {
        deps.safeTraceUpdate(
          trace,
          { input: llmInputForExport },
          `llm_input trace input sessionKey=${sessionKey}`,
        );
      }
