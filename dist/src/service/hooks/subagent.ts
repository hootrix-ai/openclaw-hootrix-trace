...
function rememberSubagentLineageFromHost(
  deps: Pick<SubagentHooksDeps, "rememberSubagentLineage">,
  childSessionKey: string,
  host: { sessionKey: string; active: ActiveTrace },
): void {
...
export function registerSubagentHooks(deps: SubagentHooksDeps): void {
  ...
  deps.api.on("subagent_ended", (event, subagentCtx) => {
    ...
    let host:
      | { sessionKey: string; active: ActiveTrace; parent: Trace | Span }
      | undefined = existingHost
      ? { sessionKey: existingHost.hostSessionKey, active: existingHost.active, parent: existingHost.span }
      : undefined;
