...
const MEDIA_IMAGE_REFERENCE_RE =
  /\bmedia:(?:https?:\/\/[^\s"'`]+|\.[/][^\s"'`]+|[/][^\s"'`]+|[^\s"'`]+)\.(?:jpe?g|png|webp|gif|mp3|wav|m4a|aac|ogg|oga|flac|opus|caf|weba|webm|mp4|mov|mkv|bmp|tiff?|heic|heif|svg)(?=[\s"'`]|$)/gi;
...
export function sanitizeStringForHootrix(value: string): string {
  ...
  const redactedInternalBlocks = normalizedNewlines
    ...
    .replace(/\n{3,}/g, "\n\n");
  const withKnownPlaceholders = normalizeMediaRefPlaceholders(
    applyRegisteredMediaPlaceholders(redactedInternalBlocks),
  );
 ...
 */
export function buildSanitizedLlmInputFromEvent(
  event: OpenClawLlmInputEventFields,
