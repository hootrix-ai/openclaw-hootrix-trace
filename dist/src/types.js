...
const PLUGIN_ENTRY_IDS = [HOOTRIX_PLUGIN_ID];
function looksLikeHootrixPluginConfig(o) {
    return ("apiKey" in o ||
        "apiUrl" in o ||
        "projectName" in o ||
        "workspaceName" in o ||
...
function shouldUseEmbeddedPluginConfig(nested) {
    if (Object.keys(nested).length === 0)
        return false;
    if (looksLikeHootrixPluginConfig(nested))
        return true;
...
export function parseHootrixPluginConfig(raw) {
    ...
    const policySyncIntervalMs = (() => {
        ...
        const envVal = process.env.HOOTRIX_POLICY_SYNC_INTERVAL_MS?.trim() ??
