...
export function createHootrixService(
  ...
  function hasOpenSubagentBridgeSpans(active: ActiveTrace): boolean {
  ...
  function forgetSubagentSpanHostsByActiveIfClosed(active: ActiveTrace): void {
  ...
  function registerHooks(): void {
    ...
    api.on("agent_end", (event, agentCtx) => {
      ...
      attachmentUploader.scheduleMediaAttachmentUploads({
        ...
        payloads: [
          event.error,
          ...(((event as Record<string, unknown>).messages as unknown[] | undefined) ?? []),
        ],
      });
  ...
  const service: HootrixRuntimeService = {
    ...
    async start(ctx) {
      ...
      setOpenClawStateDir(asNonEmptyString((ctx as Record<string, unknown>).stateDir));
