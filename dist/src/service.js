...
import { ATTACHMENT_UPLOADS_ENABLED, DEFAULT_ATTACHMENT_BASE_URL, DEFAULT_FLUSH_RETRY_BASE_DELAY_MS, DEFAULT_FLUSH_RETRY_COUNT, DEFAULT_STALE_SWEEP_INTERVAL_MS, DEFAULT_STALE_TRACE_TIMEOUT_MS, FALLBACK_FINALIZE_DELAY_MS, MAX_FLUSH_RETRY_DELAY_MS, OPIK_CREATED_FROM, OPIK_PLUGIN_ID, } from "./service/constants.js";
import { asNonEmptyString, asNonNegativeNumber, formatError, hasCostUsageFields, hasUsageFields, mergeDefinedConfig, normalizeProvider, resetOpikThreadSessionAliases, channelMetadataFields, inferChannelProviderFromThreadKey, logChannelResolve, resolveChannelName, resolveEffectiveOpikSessionKey, resolveTrigger, sleep, } from "./service/helpers.js";
...
import { parseOpikPluginConfig } from "./types.js";
...
export function createOpikService(api, pluginConfig = {}) {
    ...
    function warnMissingAfterToolSessionKey(fallbackMode) {
        ...
        log.warn(`opik: after_tool_call missing sessionKey; using ${fallbackMode} fallback correlation (upgrade OpenClaw for strict context propagation)`);
    }
    ...
    async function flushWithRetry(reason) {
        ...
        for (let attempt = 1; attempt <= attempts; attempt++) {
            try {
                ...
                log.warn(`opik: flush failed (${reason}) attempt ${attempt}/${attempts}: ${formatError(err)}`);
                if (attempt >= attempts) {
    ...
    async function validateProjectTarget(params) {
        ...
        try {
            ...
            const statusCode = typeof err === "object" && err !== null && "statusCode" in err
                ...
                : undefined;
            if (statusCode === 404) {
                log.warn(`opik: configured project "${params.projectName}" was not found in workspace "${params.workspaceName}"; traces may not appear until the project exists or the plugin is reconfigured`);
                return;
            }
            if (statusCode === 403) {
                log.warn(`opik: could not access project "${params.projectName}" in workspace "${params.workspaceName}" (forbidden); verify the API key and workspace permissions`);
                return;
            }
            log.warn(`opik: could not validate project "${params.projectName}" in workspace "${params.workspaceName}": ${formatError(err)}`);
        }
    }
    ...
    function registerHooks() {
        ...
        api.on("agent_end", (event, agentCtx) => {
            ...
            const sessionKey = resolveEffectiveOpikSessionKey(agentCtxObj) ?? resolveSessionKey(agentCtxObj);
            ...
            const active = activeTraces.get(sessionKey);
            if (!active) {
                log.warn(`opik: agent_end missing active trace sessionKey=${sessionKey} activeTraces=${activeTraces.size}`);
                return;
            }
            ...
            active.agentEnd = {
                ...
                messages: sanitizeValueForOpik(event.messages ?? []) ?? [],
            };
    ...
    const service = {
        ...
        async start(ctx) {
            ...
            if (!apiUrl?.trim()) {
                log.warn("hootrix-trace: apiUrl missing — traces will not reach collector (run `openclaw hootrix configure` or set config.apiUrl / HOOTRIX_URL)");
            }
            if (!apiKey?.trim()) {
            ...
            const workspaceName = opikCfg.workspaceName ?? trimOrUndefined(process.env.OPIK_WORKSPACE) ?? "default";
            ...
            staleTraceTimeoutMs = Math.max(1000, asNonNegativeNumber(opikCfg.staleTraceTimeoutMs) ?? DEFAULT_STALE_TRACE_TIMEOUT_MS);
            staleSweepIntervalMs = Math.max(1000, asNonNegativeNumber(opikCfg.staleSweepIntervalMs) ?? DEFAULT_STALE_SWEEP_INTERVAL_MS);
            flushRetryCount = Math.floor(asNonNegativeNumber(opikCfg.flushRetryCount) ?? DEFAULT_FLUSH_RETRY_COUNT);
            ...
            const unsubscribeDiagnosticsRaw = onDiagnosticEvent((evt) => {
                if (evt.type !== "model.usage")
                    return;
                const evtObj = evt;
                const sessionKey = resolveEffectiveOpikSessionKey(evtObj) ?? asNonEmptyString(evt.sessionKey);
                if (!sessionKey)
                    return;
                const active = activeTraces.get(sessionKey);
                if (!active)
            ...
            log.info(`opik: exporting traces to project "${projectName}" (staleCleanup=${staleTraceCleanupEnabled ? "on" : "off"}, staleTimeoutMs=${staleTraceTimeoutMs}, staleSweepMs=${staleSweepIntervalMs}, flushRetryCount=${flushRetryCount}, flushRetryBaseDelayMs=${flushRetryBaseDelayMs})`);
        },
        async stop() {
            ...
            toolResultPersistSanitizeEnabled = false;
            log.info(`opik: exporter metrics flushSuccesses=${exporterMetrics.flushSuccesses} flushFailures=${exporterMetrics.flushFailures} flushRetries=${exporterMetrics.flushRetries} traceUpdateErrors=${exporterMetrics.traceUpdateErrors} traceEndErrors=${exporterMetrics.traceEndErrors} spanUpdateErrors=${exporterMetrics.spanUpdateErrors} spanEndErrors=${exporterMetrics.spanEndErrors}`);
        },
    };
    return service;
}
