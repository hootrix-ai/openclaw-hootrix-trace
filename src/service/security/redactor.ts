...
import type { RedactionRule, RedactionType, RedactionAuditSummary, RedactionAuditFinding, RedactionAuditInterceptionMeta, LocationType } from "./types.js";
  ...
  scanObject(obj: unknown): RedactionAuditSummary {
    ...
    for (const f of findings) {
      ...
      if (action === "abort_run" || action === "input_guard") {
        maskOrMatch = true;
        if ((f as { run_aborted?: boolean }).run_aborted) {
          runBlocked = true;
        }
        continue;
      }
